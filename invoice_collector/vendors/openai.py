...
VENDOR = "openai"
DEFAULT_URL = "https://platform.openai.com/settings/organization/billing"
...
def collect_invoice(
    ...
    with PersistentBrowser(VENDOR, profiles_root, headless=headless) as (_, _, page):
        ...
        return save_first_matching_download(
            ...
            link_text_candidates=[period, period.replace("-", "/"), "Download", "Télécharger", "Invoice"],
            fallback_message=f"Aucune facture OpenAI trouvée automatiquement pour {period}. Ajustez le sélecteur dans vendors/openai.py.",
        )
